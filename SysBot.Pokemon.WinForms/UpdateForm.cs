...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        private void InitializeComponent()
        {
            ...
            if (isUpdateRequired)
            {
                labelUpdateInfo.Text = "Hay una actualización obligatoria disponible. Debes actualizar para continuar usando esta aplicación.";
                ControlBox = false;
            }
            else if (isUpdateAvailable)
            {
                labelUpdateInfo.Text = "Hay una nueva versión disponible. Por favor, descarga la última versión.";
            }
            else
            {
            ...
            buttonDownload.Size = new Size(138, 23);
        ...
        private async void ButtonDownload_Click(object? sender, EventArgs? e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("No se pudo obtener la URL de descarga. Por favor, verifica tu conexión a internet e intenta de nuevo.",
                        "Error de descarga", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error en la actualización: {ex.Message}", "Error de actualización", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
        ...
        private void InstallUpdate(string downloadedFilePath)
        {
            ...
            {
                MessageBox.Show($"Error al instalar la actualización: {ex.Message}", "Error de actualización", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
