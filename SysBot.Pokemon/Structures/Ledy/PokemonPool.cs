    ...
    where T : PKM, new()
{
    ...
    public bool LoadFolder(string path, SearchOption opt = SearchOption.AllDirectories)
    {
        ...
        foreach (var file in matchFiles)
        {
            ...
            if (!dest.CanBeTraded())
            {
                LogUtil.LogInfo("SKIPPED: El archivo proporcionado no se puede tradear: " + dest.FileName, nameof(PokemonPool<T>));
                continue;
            }
            ...
            if (!la.Valid)
            {
                ...
                LogUtil.LogInfo($"SKIPPED: El archivo proporcionado no es legal: {dest.FileName} -- {reason}", nameof(PokemonPool<T>));
                continue;
            }
